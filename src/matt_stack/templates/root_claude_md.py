...
from matt_stack.config import FrontendFramework, ProjectConfig
...
def generate_claude_md(config: ProjectConfig) -> str:
    ...
    sections.append(_matt_stack_integration(config))
...
def _rules(config: ProjectConfig) -> str:
    ...
    lines.append(
        "- **matt-stack**: `matt-stack dev` (start all), `matt-stack test`, "
        "`matt-stack lint`, `matt-stack audit`."
    )
...
def _commands(config: ProjectConfig) -> str:
    ...
    lines.append(f"matt-stack dev          # {dev_desc}")
    lines.append(f"matt-stack test         # {test_desc}")
    lines.append("matt-stack lint         # Lint all code")
    lines.append("matt-stack lint --fix   # Auto-fix lint issues")
    lines.append("matt-stack env check    # Verify .env files are in sync")
    lines.append("matt-stack audit        # Run static analysis")
    lines.append("```")
    return "\n".join(lines)
...
def _matt_stack_integration(config: ProjectConfig) -> str:
